...
% of condition (R1, R2-random, R2-best)
%
% TCS 4/13/2018
...
%the 5 subs that have more than 20% data excluded
%subj = {'sub001','sub003','sub004','sub005','sub007','sub008','sub010','sub011','sub012','sub013','sub016','sub017','sub018','sub019','sub020'};

%subjs excluded due to more negative correlation from R2-r and R2-b
%subj ={'sub003','sub004','sub005','sub006','sub007','sub008','sub011','sub012','sub013','sub014','sub016','sub017','sub018','sub019','sub021'};

%subjs excluded due to more negative correlation from R1 and R2-b
%subj = {'sub002','sub003','sub004','sub005','sub006','sub007','sub008','sub010','sub011','sub013','sub016','sub017','sub018','sub019','sub020'};
...
to_plot = {'f_sacc_err','i_sacc_rt','n_sacc'};
...
% figure; hold on;
% plot(percentPerCond_useTrial.','-'); 
% plot(mean(percentPerCond_useTrial),'k-','LineWidth',1.5);
...
for ss = 1:length(which_subj_traj)
    ...
    set(gca,'YTick',[0 6 12 15 21 27 30 36 42],'YTickLabel',{[],'R2-best',[],[],'R2-random',[],[],'R1',[]});
...
for ss = 1:length(u_subj) 
    ...
    for bb = 1:(length(my_Bin)-1)
        ...
        this_x = mean(all_data.s_all.f_sacc_err(thisidx));% compute the mean of memory error for R2-random condition
...
% ttest run for R2-random and R2-best correlation show df = 18 instead of 19,
% sub 6 nan
...
% scatterplot of Error (R2_rand - R2_best) vs all_corr for each subj (R1
% err corr w/ R2-best p_choose)
figure;
scatter(all_corr,sub_mean.f_sacc_err(:,2)-sub_mean.f_sacc_err(:,3),15,'k','filled');

xlabel('Correlation for each subj (R1 err corr w/ R2-best p(choose)');ylabel('Error (R2-random - R2-best)');


%% R2-best against R2-random precision as function of location bin  plus correlation 


all_corr2 = nan(length(u_subj),1);
...
for ss = 1:length(u_subj) % go through all the subjects
    ...
    all_corr2(ss) = corr(all_R2RmuErr(~excl_vals),all_LOC_nChoose(~excl_vals));
...
all_corr_z2 = atanh(all_corr2);


% compute mean and SEM using R-to-Z transformed values, plot these
% converted back to R (see below)
thismz2 = mean(all_corr_z2);
thisez2 = std(all_corr_z2)/sqrt(length(subj));


% plot all correlation values & their mean
figure; hold on;

plot([1.1;1.1],tanh(thismz2+[-1;1]*thisez2),'k-','LineWidth',1.5);
plot(ones(size(all_corr2))-0.1,all_corr,'ko','MarkerFaceColor','k');
plot(1.1,tanh(thismz2),'ko','MarkerSize',10,'LineWidth',1.5,'MarkerFaceColor','w');
...
% compute and spit out stats for this test
[~,thisp,~,thisstats] = ttest(all_corr_z2);
...
%% plot both R1-R2b and R2r-R2b correlation values & their mean in one
figure; hold on;
plot([0.5 2.5],[0 0],'k-','LineWidth',0.2);

plot([1.1;1.1],tanh(thismz+[-1;1]*thisez),'k-','LineWidth',1.5);
plot(1.1,tanh(thismz),'ko','MarkerSize',10,'LineWidth',1.5,'MarkerFaceColor','w');
scatter(ones(size(all_corr)),all_corr,50,cond_colors(1,:),'filled','MarkerFaceAlpha',0.5,'MarkerEdgeColor',cond_colors(1,:));

plot([2.1;2.1],tanh(thismz2+[-1;1]*thisez2),'k-','LineWidth',1.5);
plot(2.1,tanh(thismz2),'ko','MarkerSize',10,'LineWidth',1.5,'MarkerFaceColor','w');
scatter(ones(size(all_corr2))+1,all_corr2,50,cond_colors(2,:),'filled','MarkerFaceAlpha',0.5,'MarkerEdgeColor',cond_colors(2,:));



% cond_colors(1,:)
% cond_colors(2,:)
xlim([0.5 2.5]); ylim([-1 1]);xticks(1); yticks(-1:0.5:1);
set(gca,'TickDir','out','LineWidth',0.75);
ylabel('Correlation');
...
for cc = 1:length(cu)
    for ss = 1:length(u_subj)
        thisidx = all_data.subj_all==ss & all_data.c_all(:,1)==cu(cc) & all_data.use_trial==1;
        thisd(ss,cc) = mean(all_data.s_all.n_sacc(thisidx));  %redundant line
        thisd_anova(cnt) = thisd(ss,cc);
        thisX_anova(cnt,:) = [cu(cc) ss];
        cnt = cnt+1;
    end
